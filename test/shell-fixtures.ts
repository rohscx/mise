...
export class SessionMemoryArea extends MemoryArea {
...
export interface Harness extends ControllerDependencies {
  areas: StorageAreas & { local: MemoryArea; session: SessionMemoryArea; sync: MemoryArea };
  allowed: boolean;
  current: number;
...
export function harness(): Harness {
  const areas = { local: new MemoryArea(), session: new SessionMemoryArea(), sync: new MemoryArea() };
  const deps: Harness = {
