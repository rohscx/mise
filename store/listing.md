...
Run `npm run promo`, inspect the result, and upload store/promo/tile-440x280.png (440 × 280).
...
No account or credentials are needed. A fresh install includes three starter prompts. Visit https://github.com/microsoft/TypeScript, open the toolbar popup and choose Capture current tab. Select “Review a GitHub repository” and check that the preview includes microsoft/TypeScript, the shared findings instructions and the source URL. Choose Copy, then paste into a text editor. Optional tab and clipboard permissions are requested only from their explicit controls.
